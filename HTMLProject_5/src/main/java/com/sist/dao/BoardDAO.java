	...
	public List<BoardVO> boardListData(int page)
	{
		...
		try
		{
			...
			String sql="SELECT no,subject,name,regdate,hit,num "
					...
					+ "FROM (SELECT /*+ INDEX_DESC(board board_btno_pk)*/no,subject,name,regdate,hit "
					+ "FROM board)) "
	...
	public void boardInsert(BoardVO vo)
	{
		try
		{
			...
			String sql="INSERT INTO board(no,name,subject,content,pwd) "
					+ "VALUES(board_btno_seq.nextval,?,?,?,?)";
			ps=conn.prepareStatement(sql);
			ps.setString(1, vo.getName()); // 첫번째 물음표 엔 vo.getName()
			ps.setString(2, vo.getSubject());
			ps.setString(3, vo.getContent());
			ps.setString(4, vo.getPwd());
			
			// 3. SQL문장 실행 요청 
			ps.executeUpdate(); // commit 이 포함 (AutoCommit)
		}catch(Exception ex)
		{
			ex.printStackTrace();
		}
		...
		{
			disConnection(); // 반환 => 재사용 (닫는 게 아니라 재사용할 수 있게 반환)
		}
	}

...
public class GoodsDAO {
	...
	public List<GoodsVO> goodsAllListData(int page)
	{
		...
		try
		{
			...
			int rowSize=12;
			int start=(rowSize*page)-(rowSize-1); // rownum => 1 부터 시작
			...
			while(rs.next()) // rs => 단위가 Record 단위 => 여러개를 동시에 들어왔을 때 vo 에 한번에
			{
	...
	public GoodsVO goodsAllDetailData(int no)
	{
		...
		{
			conn=dbconn.getConnection();
			String sql="UPDATE goods_all SET " // 조회수 증가
					+ "hit=hit+1 "
					+ "WHERE no="+no;
			ps=conn.prepareStatement(sql);
			ps.executeUpdate();
	...
	public GoodsVO goodsCookieData(int no)
	{
		...
		{
			conn=dbconn.getConnection();
			String sql="SELECT * FROM goods_all " // 쿠키는 조회수 필요없다
					  +"WHERE no="+no;
			   ps=conn.prepareStatement(sql);
			   ResultSet rs=ps.executeQuery();
			   rs.next();

...
/*
	사용자 요청         *.do 하면 호출
	=========  ----- DispatcherServlet ----- Model 클래스 찾아주기
	 .do = 사용자 요청  | 
	                 1) Model 클래스 찾기
	                 2) 해당 메소드 찾기 역할
	                    =========
	    ...
	    => DAO (DBCP 사용중) => MyBatis 로 바뀔 예정
	    => Front (Ajax, Jquery 사용중) => VueJS 로 바뀔 예정
	       1. hosting (AWS)
	    -------------------------------------------------
	    => MyBatis => JPA (MySQL)
...
public class FoodModel {
	...
	public void foodFindDetailData(HttpServletRequest request,HttpServletResponse response)
	{
		...
		JSONObject obj=new JSONObject(); // 한개만 넘길 땐 JSONObject , 여러개는 JSONArray
		// vo 를 자바스크립트가 받을 수 있게 obj 로 넘긴다

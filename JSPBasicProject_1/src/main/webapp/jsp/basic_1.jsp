...
<%--
	   ...
	   URL
	       ...
	       ================ ==================================
	          서버 정보         요청 => URI
	                          ===============
	                                | ContextPath
	       => 파일 요청 : 경우에 따라서는 파일 확장자를 변경할 수도 있다
	                    => .회사면 => .naver , .do
	       => PathVariable /aaa/aaa => React가 PathVariable 사용 (보안 강조)
	        request(파일 요청)
    client  ======= Web Server ====== Web Container(WAS)(Web Application Server)
	======          아파치 / IIS       ================== > 자바를 번역할 수 있는 번역기 
	브라우저               |               | Tomcat       (웹서버엔 자바 jdk가 없기에)
	                  요청 받기 ==>               |
	                  ...
	                  <============               |
	                   HTML을 브라우저로 전송
	                   => 번역된 내용 : 소스 보기
	19page 요청 / 응답 => 네트워크 (C/S) (Client/Server)
	...
	1) 서블릿 
	   ...
	   => 자바안에 HTML코드를 첨부
	      out.write("<html>")...
	   => 단점 
	       = HTML을 사용하기 어렵다 (복잡) => CSS/JavaScipt
	       = 에러잡기가 어렵다
	       = 자바로 되어 있어서 약간을 수정해도 컴파일을 다시 해야 한다
	         => 버튼의 색상 변경 (문자 변경)만 하려해도 컴파일 거쳐야 한다
    ...
    => 단점을 보완 (JSP)
       ...
       => 실행을 하면 자동으로 톰캣이 컴파일을 자동 수행
          => 저장하고 브라우저를 새로 고침 (JSP)
          => 보안에 취약하다 (소스를 전체 전송 = 노출)
	                   
    => JSP의 장점
       1) 서블리보다 쉽게 소스작성이 가능하다 (빠르다)
       2) HTML / Java 가 나눠져 있다 
       3) 디자인과 소스코딩을 나눠서 작업할 수 있다
       4) 단순하다 => 데이터베이스 연동 => for / if 정도 사용
 ...
 <%--
 	...
 	3. .html , .css , .json => 웹서버 자체에서 처리 후 브라우저로 전송
 	   => jsp,servlet... 번역이 불가능 
 	      -----------
 	      |
 	     번역을 요청 => 톰캣 (resin , 제우스 , 웹로직 , 웹스페어..) => 자바 번역기
 	                 ===================================
 	     ...
 	     3) 자바 실행 
 	        =======
 	        | 브라우저에서 읽어갈 메모리에 HTML을 출력해준다
 	          ========================== 출력 스트림
 	     4) 출력된 메모리로부터 HTML을 한줄씩 읽어서 브라우저에 출력
 	                       ================= 인터프리터
 	71page => jsp의 생명주기
 	=> 자동 호출 
 	   _jspInit() => 초기화 (web.xml 등록시에 읽어 간다)
 	      |
 	***_jspService() => JSP에서 코딩이 되는 영역 (메소드 내부 영역)
 	      |             화면 / 결과를 출력
 	   _jspDestroy() => 새로고침 / 화면 이동 하면 자동으로 메모리 해제

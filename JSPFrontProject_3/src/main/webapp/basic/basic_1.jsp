		...
		JavaScript : 브라우저에 처리 (정적 => 동적)
		     ...
		     *** 자바와 자바스크립트 연동
		         ~VO   === {} => Object
		                   == 자바스크립트 객체 표현법 => JSON
		         ~List === [{}.{}.{}...] => Array
		   => 브라우저안에서 자바에서 전송된 데이터를 받아서 동적 변환 => JavaScript
		                        =========== Array,Object
		   => 자바스크립트 사용법
		      1) 내부 스크립트 (파일 한개 제어)
		         <head>
		      ...
		      3) 인라인 스크립트 : 태그 한개 제어
		         ...
		         function aaa()
		         <a href="aaa()"> (X)
		         <a href="javascript:aaa()"> (O) => 우편번호 검색 시 사용
		  => 버전
		     ES5 ==> ES6
		     변수 ==> 자동 지정 변수
		             var => let , const
...
/*
	=> 변수 
	1) 변수 선언 (데이터형이 없다) => 값에 따라서 자동으로 데이터형이 변경된다
	   var => let , 상수 => const
	   var의 단점은 지역변수의 스코프가 명확하지 않다
	   function aaa()
	   {
	   	   var a=10; => var : 메모리 누수 현상이 많이 존재
	   } // 메모리가 해제 (X) var는 구버전
	   alert(a); < 오류 없다
...
window.onload=function(){
	/*
		출력 방법 => System.out.println() (자바)
		=> console.log(출력물) (콘솔창에서만)
	...
	*/	
	...
	let m; // 값이 없는 경우 / 선언안된 변수가 있는 경우 => undefined
